...
(function () {
  ...
  function initPanel(panel) {
    ...
    // data-proxy: the CORS-safe forwarding endpoint (Cloudflare Worker) that
    // this panel's messages actually get sent to. data-webhook stays the
    // REAL n8n URL, used only for the "open in new tab" button so that link
    // keeps going straight to the real agent. If data-proxy isn't set on a
    // panel, that panel falls back to calling data-webhook directly.
    var fetchTarget = panel.getAttribute('data-proxy') || webhook;
